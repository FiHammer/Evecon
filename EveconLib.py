...
class MusicPlayerC(threading.Thread):
    ...
    def queueByFile(self, plfile):
        res = Search(plfile, self.playlist)
        if not res:
            oldPL = self.playlist.copy()
            del oldPL[0]
    ...
    def react(self, inp):
        ...
        # Search BLOCK
            ...
            if inp == inp.lower(): # SEARCH
                ...
                if i == "p":
                    oldPL = self.playlist.copy()
                    del oldPL[Search(self.searchlist[self.cur_Pos], self.searchlist)[0]]
...
class MusicFileEditor:
    def __init__(self, specialFilePath=None):
        self.minVersion = "1.1"
        self.newestVersion = "1.1"
        self.forbiddenIDs = ["names", "ids"]

        self.nFilePath = "data" + path_seg + "Config" + path_seg + "Music.json"  # normal file path
        self.bFilePath = "data" + path_seg + "Backup" + path_seg + "Music.json"  # backup file path
    ...
    def decodeJson(self, data):
        ...
        if data.get("version"):
            if not data["version"] == self.newestVersion:
                return errorPrinter("Wrong Version! (" + version + " required)")
        else:
    ...
    def encodeData(self, musicDirs, multiPls, genre, musicDir, versionX, pcX):
        ...
        if not self.minVersion <= versionX <= self.newestVersion:
            return errorPrinter(
                "Version out of range (Min: %s, Max: %s, Data's: %s" % (self.minVersion, self.newestVersion, versionX))
