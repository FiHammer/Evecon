...
def readJson():
    global pageurl, data
    with open(EveconLib.Config.foxiDir+"website.txt") as file:
        pageurl = file.readline().rstrip()
    with open(EveconLib.Config.foxiDir+"data.json") as jsonfile:
        data = json.load(jsonfile)

def writeJson():
    with open(EveconLib.Config.foxiDir+"data.json", "w") as jsonfile:
...
def fap(opentype="fox"):
    ...
    print("Which is your startpage? (Begin: %s, Search for: %s)" % (data["Last"]["last_page"], idstart))
    pagestart = int(input())
